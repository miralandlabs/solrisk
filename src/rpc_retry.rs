...
impl RetryPolicy {
    ...
    /// - `SOLRISK_RPC_MAX_ATTEMPTS` — default **3**
    /// - `SOLRISK_RPC_INITIAL_BACKOFF_MS` — default **120**
    /// - `SOLRISK_RPC_TOTAL_BUDGET_MS` — default **6000**
    pub fn from_env() -> Self {
        ...
        let max_attempts = env_u64("SOLRISK_RPC_MAX_ATTEMPTS", 3).clamp(1, 10) as u32;
        let initial_ms = env_u64("SOLRISK_RPC_INITIAL_BACKOFF_MS", 120).clamp(10, 5_000);
        let total_ms = env_u64("SOLRISK_RPC_TOTAL_BUDGET_MS", 6_000).clamp(100, 30_000);
        Self {
            max_attempts,
